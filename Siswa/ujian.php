...
if ($rujian['acak'] == 'acak') {
  $qsoal  = mysqli_query($mysqli, "SELECT * FROM soal_essay 
      WHERE id_ujian  = '$_GET[ujian]' 
      ORDER BY rand() 
      LIMIT $rujian[jml_soal]
    ");
} else {
  $qsoal = mysqli_query($mysqli, "SELECT * FROM soal_essay 
      WHERE id_ujian  = '$_GET[ujian]' 
...
$q2soal = mysqli_query($mysqli, "SELECT * FROM soal 
    WHERE id_ujian  = '$_GET[ujian]' 
    ORDER BY id_soal
  ");
...
if (mysqli_num_rows($qnilai) < 1) {
  ...
  mysqli_query($mysqli, "INSERT INTO nilai 
      SET id_siswa      = '$_SESSION[id_siswa]',
          id_ujian      = '$_GET[ujian]', 
          acak_soal     = '$acak_soal', 
          jawaban       = '$jawaban', 
          sisa_waktu    = '$waktu',
  ...
  foreach ($kls as $kelas) {
    mysqli_query($mysqli, "INSERT INTO analisis 
        SET id_siswa  = '$_SESSION[id_siswa]',
            id_ujian  = '$_GET[ujian]', 
            id_soal   = '$kelas', 
            jawaban   = '0'
  ...
  if ($mn < 0) {
    mysqli_query($mysqli, "UPDATE nilai 
        SET sisa_waktu  = '00:00:01' 
        WHERE id_siswa  = '$_SESSION[id_siswa]' 
        AND id_ujian    = '$_GET[ujian]'
      ");
  } else {
    mysqli_query($mysqli, "UPDATE nilai 
        SET sisa_waktu = '$hr:$mn:$sc' 
        WHERE id_siswa  = '$_SESSION[id_siswa]' 
        AND id_ujian  = '$_GET[ujian]'
      ");
  }
...
for ($s = 0; $s < count($arr_soal); $s++) {
  $rsoal  = mysqli_fetch_array(mysqli_query($mysqli, "SELECT * FROM soal_essay 
      WHERE id_soal='$arr_soal[$s]'
    "));
  ...
  $active = ($no == 1) ? "active" : "";
  if ($rsoal['gambar'] !== null || !empty($rsoal['gambar'])) {
    $gambar_soal = '<img src="../vendor/images/img_Soal/' . $rsoal['gambar'] . '" width="100px">';
  } else {
    $gambar_soal = '';
  }
  echo ' 
      <div id="picture">
        <div class="blok-soal soal-' . $no . ' ' . $active . '">
          <div id="fontlembarsoal" class="fontlembarsoal">
            <span id="hurufsoal"> Soal Nomor :  <a id="jfontsize-d2" style="font-size: 16px; text-decoration: none; cursor: pointer;">&nbsp; ' . $no . ' &nbsp;</a> </span>
          </div>   
          <div id="lembaran">
            <div id="lembaransoal">
                  ...
                  <tr>
                    <td valign="top">
                      <div class="form-group">
                        ...
                        <textarea class="jawabanSiswa" name="jawaban" id="jawaban' . $s . '" required>' . $arr_jawaban[$s] . '</textarea>
                        <script>
                          CKEDITOR.replace("jawaban' . $s . '", {
                            toolbar: [{
                              name: "basicstyles",
                          ...
                          CKEDITOR.instances.jawaban' . $s . '.on("change", function(event) { 
                            var ujian = $("#ujian").val();
                            var jawaban = this.getData();
                            if (jawaban) {
                              $.ajax({
                                url: "ajax_ujian.php?action=kirim_jawaban",
                                type: "POST",
                                data: "ujian=" + ujian + "&index=" + ' . $s . ' + "&jawab=" + jawaban,
  ...
  if ($no != 1) echo ' 
      <div>
        <a onclick="tampil_soal(' . $sebelumnya . ')">            
  ...
  // </div>
  // <div>
  //   <label class="labele" style="padding-bottom:10px; padding-top:10px; width:225px">
  ...
  <div class="modal-dialog" role="document">
    ...
    <div class="modal-content">
      <div class="modal-header">
       ...
       <form  onsubmit="return selesai_ujian_essay(' . $_GET['ujian'] . ')">
      ...
      <div class="modal-footer">
        <button type="submit" class="btn btn-success"  onclick="return selesai_ujian_essay(' . $_GET['ujian'] . ')">SELESAI</button>
...
<!-- <textarea name="jawaban" id="jawaban' . $s . '" required oninput="kirim_jawaban_essay(' . $s . ')">' . $arr_jawaban[$s] . '</textarea>
<script>
  CKEDITOR.replace("jawaban' . $s . '", {
    filebrowserImageBrowseUrl: "../vendor/kcfinder"
  });
