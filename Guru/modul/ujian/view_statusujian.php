<div class="content-wrapper">
  ...
  $ujian = mysqli_query($con, "SELECT * FROM ujian INNER JOIN tb_master_mapel ON ujian.id_mapel=tb_master_mapel.id_mapel INNER JOIN tb_jenisujian ON ujian.id_jenis=tb_jenisujian.id_jenis INNER JOIN tb_master_semester ON ujian.id_semester=tb_master_semester.id_semester WHERE ujian.id_guru='$data[id_guru]' AND ujian.id_ujian='$_GET[id]'");

  $jm = mysqli_num_rows($ujian);
  foreach ($ujian as $d) {
    // tampilkan kelas ujian
    $kelas = mysqli_query($con, "SELECT * FROM kelas_ujian INNER JOIN tb_master_kelas ON kelas_ujian.id_kelas=tb_master_kelas.id_kelas WHERE kelas_ujian.id_ujian='$_GET[id]' AND kelas_ujian.aktif='Y' ");
