  ...
  <?php } else { ?>
    <div class="content-wrapper">
      ...
      foreach ($jenis as $jes) {
        $mapel  = mysqli_fetch_assoc(mysqli_query($con, "SELECT id_tugas FROM tb_tugas  WHERE id_mapel = '" . $jes['id_mapel'] . "' "));
        if ($mapel['id_tugas'] == null || empty($mapel['id_tugas'])) {
          $status_tugas = true;
          $tugas = $tugas . " <strong>" . $jes['mapel'] . ', </strong>';
        }
      }
      ?>
      <?php if ($status_tugas == true) : ?>
        <div class="alert alert-danger ?>" role="alert">
          Mata Pelajaran yang tidak ada tugas <?= $tugas ?>
        </div>
      <?php endif; ?>
      <div class="row">
        <div class="col-md-12">
